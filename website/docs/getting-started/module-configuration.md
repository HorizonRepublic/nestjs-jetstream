...
| `onDeadLetter` | `(info: DeadLetterInfo) => void \| Promise<void>` | _(none)_ | Callback for dead letter handling. Called (and awaited if async) when a message exhausts all delivery attempts. <Since version="2.2.0" /> |
